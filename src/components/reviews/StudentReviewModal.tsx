...
export function StudentReviewModal({
  ...
  const [qualityRating, setQualityRating] = React.useState(5); // Professionalism
  const [timelinessRating, setTimelinessRating] = React.useState(5);  // Payment promptness
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      await reviewService.submitReview(studentId, workflowId, {
        rating,
        communicationRating,
        qualityRating,
        timelinessRating,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-brand-hairline px-6 py-5 bg-white">
          <div>
            <h3 className="text-base font-medium text-brand-ink tracking-tight">Rate client Collaboration</h3>
            <p className="text-[11px] text-brand-muted mt-0.5">Evaluate your working relationship with {businessName}</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            <RatingField 
              label="Overall Collaboration" 
              description="How was your experience working with this business?" 
              value={rating} 
              onChange={setRating} 
              isPrimary
            />
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-bold uppercase tracking-wider text-brand-muted">Collaboration Experience (Optional)</label>
            <textarea
              value={reviewText}
              onChange={(e) => setReviewText(e.target.value)}
              rows={4}
              placeholder="What was it like collaborating? Comment on requirements clarity, professionalism, payment speed..."
